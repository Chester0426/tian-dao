...
const ITEM_DISPLAY: Record<string, { name: string; icon: string; color: string; rarity: string }> = {
  煤: { name: "煤", icon: "◆", color: "text-ink-2", rarity: "common" },
  銅礦: { name: "銅礦", icon: "◇", color: "text-spirit-gold", rarity: "uncommon" },
  靈石碎片: { name: "靈石碎片", icon: "✦", color: "text-jade", rarity: "rare" },
};

const INITIAL_SLOTS = 20;

export default function InventoryPage() {
  // In a real app this would come from Supabase; demo mode uses empty state
  const [inventory] = useState<InventoryItem[]>([]);
  const [totalSlots] = useState(INITIAL_SLOTS);
