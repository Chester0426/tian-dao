...
export default async function MiningPage() {
  ...
  // Fetch mine data and player state in parallel
  const [mineRes, profileRes, miningSkillRes, masteryRes, inventoryRes] = await Promise.all([
    supabase.from("mines").select("*").eq("slug", "depleted_vein").single(),
  ...
  // Add action_interval_ms to mine data (not in DB, hardcoded for now)
