      ...
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <div className="grid gap-4 grid-cols-2 lg:grid-cols-4 auto-rows-fr">
          {mines.map((mine) => {
            ...
            const depletedAt = gameState.rockDepletedAtMap[mine.id];
            // Server-authoritative HP display. No client regen formula —
            // server's mine_regen_tick updates rockHpMap every 10s.
            // Respawning mine: visually fill HP smoothly from 0 to max over respawn_seconds.
            let currentRockHp: number;
            let rockHpPct: number;
            const currentRockMaxHp = mineMaxHp;
            if (savedHp === undefined) {
              currentRockHp = mineMaxHp;
              rockHpPct = 100;
            } else if (depletedAt) {
              const respawnMs = mine.respawn_seconds * 1000;
              const elapsed = Date.now() - depletedAt;
              const progress = Math.min(elapsed / respawnMs, 1);
              currentRockHp = Math.floor(progress * mineMaxHp);
              rockHpPct = progress * 100;
            } else {
              currentRockHp = Math.min(savedHp, mineMaxHp);
              rockHpPct = (currentRockHp / mineMaxHp) * 100;
            }
            ...
            return (
              ...
              >
                <div className="p-3 space-y-2.5">
                  ...
                  {(() => { const isMiningActive = isActive && !depletedAt; return (
                  ...
                  {/* Rock HP bar — unified: shows live HP (with regen + respawn progress) */}
                  ...
                  <div className="h-2 w-full overflow-hidden rounded-full" style={{ background: "rgba(0,0,0,0.4)" }}>
                    <div
                      className="h-full rounded-full"
                      style={{
                        width: isActive && !depletedAt ? `${actionProgress}%` : "0%",
                        background: "linear-gradient(90deg, #1a4a3a, #3ecfa5, #6ee7b7)",
