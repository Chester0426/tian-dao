...
export function EnlightenmentClient({
  ...
  const [learnedTechniques, setLearnedTechniques] = useState<LearnedTechnique[]>(initialLearned);

  // Sync learnedTechniques when initialLearned prop changes
  useEffect(() => { setLearnedTechniques(initialLearned); }, [initialLearned]);
  ...
  const [currentTarget, setCurrentTarget] = useState<Target | null>(initialTarget ?? null);
  const [isEnlightening, setIsEnlightening] = useState(!!initialTarget && gameState.isEnlightening);
  ...
  // Register no-op sync function (per-tick RPC handles persistence)
  useEffect(() => {
  ...
  const learnedMap = new Map(learnedTechniques.map((t) => [t.technique_slug, t]));
  ...
  // Server-authoritative tick: per-tick RPC, no batch sync
  const doEnlightenmentTick = () => {
    supabaseRef.current.rpc("enlightenment_tick", { p_slot: 1 }).then(({ data, error }: { data: unknown; error: unknown }) => {
      ...
      const result = data as {
        ...
        enlightenment_xp?: number;
        enlightenment_level?: number;
        enlightenment_xp_gained?: number;
        mastery_xp_gained?: number;
        mastery_level?: number;
        mastery_xp?: number;
        leveled_up_enlight?: boolean;
        leveled_up_mastery?: boolean;
        out_of_books?: boolean;
        maxed?: boolean;
      };
      if (result.error) return;
      if (!isEnlighteningRef.current) return;

      // Update XP from server
      if (typeof result.enlightenment_xp === "number") setEnlightenmentXp(result.enlightenment_xp);
      if (typeof result.enlightenment_level === "number") setEnlightenmentLevel(result.enlightenment_level);

      // Update technique mastery from server (technique tick path)
      const tgt = currentTargetRef.current;
      if (tgt?.kind === "technique" && result.kind === "technique") {
        const slug = (tgt as { technique_slug: string }).technique_slug;
        if (typeof result.mastery_level === "number" && typeof result.mastery_xp === "number") {
          const newLevel = result.mastery_level;
          const newXp = result.mastery_xp;
          setLearnedTechniques((prev) => {
            const idx = prev.findIndex((t) => t.technique_slug === slug);
            if (idx >= 0) {
              const updated = [...prev];
              updated[idx] = { ...updated[idx], mastery_level: newLevel, mastery_xp: newXp };
              return updated;
            }
            return prev;
          });
          // Mastery level-up notification
          if (result.leveled_up_mastery && !document.hidden) {
            const tech = TECHNIQUES[slug];
            const name = tech ? (isZh ? tech.nameZh : tech.nameEn) : slug;
            gameState.addNotification("⭐", isZh ? `${name} 精通` : `${name} Mastery`, newLevel, "text-jade");
          }
        }
      }

      // Update inventory: decrement consumed book + add drops
      const target = currentTargetRef.current;
      gameState.updateInventory((prev) => {
        let updated = [...prev];
        // Book path: decrement consumed book
        if (target?.kind === "book" && result.kind === "book") {
          const idx = updated.findIndex((i) => i.item_type === target.item_type);
          if (idx >= 0) {
      ...
      // Notification: XP
      if (target && !document.hidden && result.enlightenment_xp_gained) {
        const label = target.kind === "book"
          ? (isZh ? "參悟" : "Enlightenment")
          : (isZh ? TECHNIQUES[(target as { technique_slug: string }).technique_slug]?.nameZh ?? "功法" : "Technique");
        gameState.addNotification("📜", label, result.enlightenment_xp_gained, "text-spirit-gold");
      }

      // Notification: drops
      if (result.drops && result.drops.length > 0 && !document.hidden) {
        for (const drop of result.drops) {
          const info = ITEMS[drop.item];
          if (!info) continue;
          gameState.addNotification(info.icon ?? "○", isZh ? info.nameZh : info.nameEn, drop.qty, info.color ?? "text-foreground", undefined, info.image);
        }
      }

      // Out of books or maxed → stop session, refresh
      if (result.out_of_books || result.maxed) {
        setIsEnlightening(false);
  ...
  // RAF tick loop — calls RPC every 5s
  useEffect(() => {
  ...
  const stopEnlightenment = () => {
    ...
    gameState.setEnlightening(false);
    setCurrentTarget(null);
    // Per-tick RPC has already persisted everything — just stop the session
                ...
                <div className="min-w-0 space-y-3 flex flex-col justify-center">
                  ...
                  <div className="space-y-1.5 text-sm">
                    {[
                      ...
                      { label: isZh ? "每次經驗" : "XP / tick", value: currentTarget?.kind === "book" ? "100" : "10", unit: isZh ? "點" : "p" },
                    ].map((row) => (
                ...
                <div className="relative z-50 mt-6 mx-auto max-w-md rounded-xl border border-spirit-gold/20 bg-card shadow-2xl overflow-hidden">
                  ...
                  <div className="p-4 space-y-4 max-h-[50vh] overflow-y-auto">
                    ...
                    <div>
                      ...
                      {(() => {
                        const unmaxed = learnedTechniques.filter((t) => t.mastery_level < MAX_MASTERY_LEVEL);
                        if (unmaxed.length === 0) {
