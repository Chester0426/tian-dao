...
1. Read `.claude/<skill>-context.json` for `skill` and `run_id`
...
```bash
RUN_ID=$(python3 -c "import json; print(json.load(open('.claude/<skill>-context.json')).get('run_id', ''))" 2>/dev/null || echo "")
