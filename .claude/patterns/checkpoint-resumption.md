...
When `.claude/current-plan.md` exists with YAML frontmatter, skills resume at the
exact checkpoint without re-deriving classification, scope, or stack.

## Protocol

1. Check if `.claude/current-plan.md` exists
...
| `phase2-step6` | STATE 9 (update specs) |
| `phase2-step7` | STATE 10 (implement) |
