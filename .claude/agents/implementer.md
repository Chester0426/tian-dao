...
**Visual task handling**: If the task prompt includes visual design guidelines (injected by the caller for `.tsx` page/component tasks), read `.claude/patterns/design.md` and `src/app/globals.css` (if it exists) for theme tokens. Apply these guidelines during the GREEN phase — visual quality is built in, not bolted on.
