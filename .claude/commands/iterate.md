...
- Verify the app is deployed: check for `.claude/deploy-manifest.json`. If the file does not exist, read the archetype from experiment.yaml and warn with archetype-appropriate guidance:
  - **web-app / service**: "No deployment detected — analytics only tracks live traffic. If you haven't deployed yet, run `/deploy` first, wait for traffic, then re-run `/iterate`. If you've deployed without using `/deploy` (manual deploy), you can proceed with manual funnel numbers in Step 2b."
