-- combat_tick: cap simulation window to 12 hours per call.
--
-- Mirrors mine_action's anti-timeout behavior. Without a cap, returning from a
-- multi-day offline session would force the RPC to simulate tens of thousands
-- of attack events in a single transaction — risking Supabase RPC timeout
-- and OOM under jsonb manipulations.
--
-- After cap: combat_tick processes at most 12 hours per invocation. If more
-- time has elapsed, last_sync_at is advanced by 12h (not to now), so the
-- backlog drains across subsequent combat_tick calls. Player still gets the
-- full reward over multiple ticks; just split across calls.
...
BEGIN
  ...
  v_simulate_until := LEAST(
    v_now,
    COALESCE(v_session.last_sync_at, v_session.started_at) + interval '12 hours'
  );
  ...
  LOOP
    ...
    EXIT WHEN v_event_at > v_simulate_until;
  ...
  IF v_died THEN
    UPDATE idle_sessions SET payload = v_payload, last_sync_at = v_simulate_until, ended_at = v_simulate_until
    WHERE id = v_session.id;
    v_next_event_ms := -1;
  ELSE
    UPDATE idle_sessions SET payload = v_payload, last_sync_at = v_simulate_until
    WHERE id = v_session.id;
    v_next_p_attack := v_last_p_attack + v_player_attack_interval;
    v_next_m_attack := v_last_m_attack + v_attack_speed_interval;
