...
- service: REACH = API adoption rate, DEMAND = integration requests, MONETIZE = API key upgrades, RETAIN = monthly active integrations
- cli: REACH = install rate, DEMAND = daily active usage, MONETIZE = pro feature adoption, RETAIN = update rate
...
```json
{
  ...
  "bottleneck_dimension": "REACH|DEMAND|MONETIZE|RETAIN",
...
2. Hypothesis Generation Rules — 每个 funnel dimension 至少一个假设。Category 匹配 dimension（REACH→CTR, DEMAND→signup rate, MONETIZE→payment conversion, RETAIN→return rate）。Threshold 来自行业基准。Priority 0-100。Dependencies 追踪。
3. Variant Differentiation Criteria — 最少 3 个 variants。每个 unique angle。不允许共享 primary pain point。Slug: lowercase-hyphenated。
4. Funnel Threshold Derivation — L1: REACH + DEMAND. L2: + MONETIZE. L3: + RETAIN。阈值 level-adjusted。
...
- 4 维度（REACH, DEMAND, MONETIZE, RETAIN）
...
- 检测方式: Vercel Cron 或 Stripe billing cycle webhook，检查 active experiment count vs plan allowance
...
Verdict 产生后:
a. Write experiment_decisions row
...
- PostHog server events: platform_margin_weekly { blended_margin_pct, hard_limit_hit_rate, auto_fix_rate, total_revenue_cents, total_cost_cents }
...
10. Cron routes 存在（metrics-sync, cleanup, notifications, cost-monitor）
...
| Notifications | S11 | GET /api/notifications, PATCH /api/notifications/:id |
| Webhooks | S8 | POST /api/webhooks/stripe |
| Cron | S11 | /api/cron/metrics-sync, /api/cron/cleanup, /api/cron/notifications, /api/cron/cost-monitor |
